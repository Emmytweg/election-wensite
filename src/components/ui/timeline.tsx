...
interface TimelineProps {
  data: TimelineEntry[];
  lineColor?: string; // optional gradient color
}

export const Timeline = ({ data, lineColor = "from-purple-500 via-blue-500" }: TimelineProps) => {
  const ref = useRef<HTMLDivElement>(null);
  const containerRef = useRef<HTMLDivElement>(null);
  const [height, setHeight] = useState(0);

  // Dynamic height using ResizeObserver
  useEffect(() => {
  ...
  // Scroll progress debug (optional)
    ...
    >
      <div className="max-w-7xl mx-auto py-20 px-4 md:px-8 lg:px-10">
        ...
        <p className="text-neutral-700 dark:text-neutral-300 text-sm md:text-base max-w-sm">
          Get to know the latest update about the current election
        </p>
      </div>
      ...
      <div ref={ref} className="relative max-w-7xl mx-auto pb-20">
        ...
        >
          <motion.div
            ...
            className={`absolute inset-x-0 top-0 w-[2px] bg-gradient-to-t ${lineColor} to-transparent rounded-full`}
          />
        </div>
      </div>
    </section>
